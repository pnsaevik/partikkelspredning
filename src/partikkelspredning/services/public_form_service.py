...
at the repo root), not an HTTP endpoint - unrelated to the forms registry's
`GET /`/`GET /forms` (see `services.form_registry`/`services.form_renderer`).
Reuses `form_renderer.generate_form_html` for the HTML itself; the field
set comes from `domain.public_forms.PUBLIC_FORM_PARAMETERS`.
"""
